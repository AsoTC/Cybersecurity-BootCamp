...
If your class covered security headers, what security request headers have been included? Strict-Transport-Security,X-Content-Type, X-Frame-Options, X-XSS_Protection
...
Why would we never run DELETE FROM <table-name>; by itself? Without a WHERE clause the DELETE statement will remove all the data in the table. Then you get to go talk to the DBA about restoring data from trasnaction logs and making them grumpy.
